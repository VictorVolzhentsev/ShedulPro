...
class Database:
    ...
    async def add_user(self, user_id):
        await self.connection.execute("INSERT INTO users (user_id) VALUES (?)", (user_id,))
        await self.connection.commit()
    ...
    async def set_notification_status(self, user_id, status: bool):
        ...
        await self.connection.execute("UPDATE users SET notifications_enabled = ? WHERE user_id = ?", (val, user_id))
        await self.connection.commit()

    async def get_users_with_notifications(self):
        """Returns list of users with notifications enabled and group selected"""
        async with self.connection.execute("SELECT user_id, group_id, language FROM users WHERE notifications_enabled = 1 AND group_id IS NOT NULL") as cursor:
